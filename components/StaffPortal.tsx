...
import { X, CheckCircle, Send, User, Phone, Lock, KeyRound } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/95 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
        ...
        {/* Header */}
        <div className="bg-[#0B2F5C] p-6 text-white flex justify-between items-center">
          <div>
            <h2 className="text-xl font-bold flex items-center gap-2">
              {isAuthenticated ? <User size={24} className="text-blue-400" /> : <Lock size={24} className="text-blue-400" />}
              {isAuthenticated ? 'Staff Dashboard' : 'Staff Login'}
            </h2>
            <p className="text-blue-200 text-xs mt-1">
              {isAuthenticated ? 'Send Work Completion Report' : 'Restricted Area'}
            </p>
          </div>
        ...
        {!isAuthenticated ? (
          ...
          <form onSubmit={handleSendReport} className="p-6 space-y-4 animate-in slide-in-from-bottom-4 duration-300">
            ...
            <button 
              type="submit" 
              className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 rounded-lg shadow-lg flex items-center justify-center gap-2 mt-2 transition-transform active:scale-95"
            >
              <Send size={18} />
              Send Clearance on WhatsApp
            </button>
